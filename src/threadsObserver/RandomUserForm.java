...
//Q1    We're using threads inprograms when we need simultanious tasks to execute at 
//      the same time without having to worry about the rest of the program pausing
//Q2    The race condition problem is when it matters what thread gets first to executing a method. 
//      We solve this by making methods synchronized, making atomic values and so on.  
//      This ensures consistency and that the result is as expected.
//Q3    Threads helps us making user interfaces responsive by moving execution of methods 
//      away from the main thread. This ensures that the UI is not freezing while working out it's magic.
//Q4    
//
//Q5    The problem related to pressing the button is, that we can't use the rest of the program while
//      it executes it's work/methods.
